...
function findMarker(array, dist){
    for(let i = dist; i < array.length; i++){
        let subSet = new Set(array.slice(i-dist, i));
        if(i < 20) console.log(`${i} ${array.slice(i-dist, i)} ${subSet.size}`);
        if(subSet.size == dist){
            return i
        }
    }
...
function executePart1(dataset) {
    return findMarker(dataset.split(''), 4);
}

function executePart2(dataset) {

    return findMarker(dataset.split(''), 14);
}
